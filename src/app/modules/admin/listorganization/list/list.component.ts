...
export class OrganizationListComponent implements OnInit, OnDestroy {
            ...
            .subscribe((lstOrganization: any[]) => {
                ...
                if (lstOrganization && lstOrganization.length > 0) {
                    ...
                    lstOrganization.forEach((obj) => {
                        if (obj?.ORGID_PARENT == null || !this.checkExistsOrg(obj.ORGID_PARENT, lstOrganization)) {
                            lstTreeDataOrganization.push({
                                object: obj,
                                children: this.getChildObjectByParent(obj.ORGID, lstOrganization)
                            })
                        }
                    })
    ...
    checkExistsOrg(orgid, lstOrg): boolean {
        let bResult: boolean = false;
        if (lstOrg && lstOrg.length > 0) {
            lstOrg.forEach((obj) => {
                if (obj?.ORGID == orgid) {
                    bResult = true;
                }
    ...
    addNewGroup() {
        this._functionService.isInsert().subscribe((auth: boolean) => {
            if (auth) {
                this._listorganizationService.createObject({
